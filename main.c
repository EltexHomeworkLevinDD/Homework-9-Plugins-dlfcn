...
// Всегда постоянные параметры (и для исходников библиотек)
...
#define L_NAME_PREFIX           "./lib/lib"             // Префикс в имени библиотеки [directory]/[prefix][library][postfix] 
#define L_NAME_PREFIX_LEN       9                       // Длина префикса в имени библиотеки (длина L_NAME_PREFIX без учета '\0')
#define L_NAME_POSTFIX          ".so"                   // Постфикс в имени библиотеки [directory]/[prefix][library][postfix]      
#define L_NAME_POSTFIX_LEN      3                       // Длина постфикса в имени библиотеки (длина L_NAME_PREFIX без учета '\0')
...
struct Library{
    ...
    const char short_name[L_NAME_LENGTH+1];             // Короткое имя библиотеки (без префикса и постфикса)
    int (*function)(int,int);                           // Указатель на функцию библиотеки
    const char function_name[L_FUNCTION_NAME_LEN+1];    // Имя функции в библиотеке
    const char description[L_DESCRIPLION_LENGTH+1];     // Описание функции в библиотеке
...
int main(){
    ...
    memset(libraries, '\0', sizeof(libraries));     // Зануляем поля структуры
    ...
    char function_name_var[L_FUNCTION_NAME_LEN + 1];    // Имя строковой переменной, содержащей имя функции
    char description_var[L_DESCRIPLION_LENGTH+1];       // Имя строковой переменной, содержащей описание функции
    ...
    while (libraries_count < MAX_L_COUNT){ 
        ...
        if (fgets(lib_short_name, L_NAME_LENGTH+1, stdin) == NULL)   // Считываю строку
            EXITMSG("Error entering library name\n");
        // Если пользователь ввел выход, закрыть ввод
        ...
        // Копируем строку char с коротким именем библиотеки в поле структуры с const char короткого имени библиотеки
        memcpy((void*)libraries[libraries_count].short_name, (void*)lib_short_name, sizeof(lib_short_name)); 
        
        // Создать строку полного имени библиотеки
        strcpy(lib_full_name, L_NAME_PREFIX);
        strcat(lib_full_name, lib_short_name);
        strcat(lib_full_name, L_NAME_POSTFIX);
        // Копипуем строку char полного имени библиотеки в поле структуры с const char 
        ...
        // Определяем имя переменной, содержащей имя функции
        strcat(function_name_var, lib_short_name);
        strcat(function_name_var, L_FUNCTION_NAME_POSTFIX);

        // Получаем переменную, содержащую имя функции
        ...
        // Копируем строку char с именем функции в поле структуры с const char 
        memcpy((void*)libraries[libraries_count].function_name, (void*)function_name_var_ptr, L_FUNCTION_NAME_LEN); 

        // Получаем void указатель на функцию библиотеки
        ...
        // Определяем имя переменной, содержащей описание функции
        strcat(description_var, lib_short_name);
        strcat(description_var, L_DESCRIPTION_NAME_POSTFIX);

        // Получаем переменную, содержащую описание функции
        ...
        // Копируем char описание функции в поле структуры с const char 
    ...
    while(work){
        ...
        // Если введена недоступная команда
        if (command > libraries_count){
            ...
            if (command == 0){
                ...
                // Если введена команда выполнения
